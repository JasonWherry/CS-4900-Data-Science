...
Clustering Part 2:
	Tasks:
		- get rid of irrelevant NPs: any with less than 20 windows present
		- Look for pattern in second iteration
...
def plotClusters(clusters, clust1, clust10, clust2, clust3):
	...
	# tempList = []
	# for NP in clust10.keys():
	# 	tempList.append(NP)

	# plt.yticks(tempList)
	# plt.set_yticks(tempList, minor=True)
	plt.show()
...
for item in seg:
	if sum(item) > 10:
		...
		for item in seg:
			if sum(item) > 10: # changed from 0 to 10
				pass
			else:
				seg.remove(item)
